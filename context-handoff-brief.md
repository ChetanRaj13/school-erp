...
- Fixed compiler crashes during release builds on constrained Vercel container instances by exporting `NODE_OPTIONS="--max-old-space-size=4096"` and reducing compiler footprint via `--dart2js-optimization O1`. The default `O4` optimization level was causing silent `dart2js` out-of-memory crashes on Vercel's hobby tier.
